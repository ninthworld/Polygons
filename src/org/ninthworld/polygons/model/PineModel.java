...
public class PineModel {
    ...
    private static float randSD(Random r, float sd){
        return ((r.nextFloat() * 2f) - 1f) * sd;
    }

    public static RawModel generatePineRawModel(){
        ...
        for(int s=0; s<trunkSections; s++){
            for(int i=0; i<trunkNGon; i++){
                ...
                float y = -0.2f;
                if(s > 0) y += trunkHeight[s-1];

                Vector3f v0 = new Vector3f((float) Math.sin(a0) * trunkRadius[s], y, (float) Math.cos(a0) * trunkRadius[s]);
                Vector3f v1 = new Vector3f((float) Math.sin(a1) * trunkRadius[s], y, (float) Math.cos(a1) * trunkRadius[s]);
                Vector3f v2 = new Vector3f((float) Math.sin(a1) * trunkRadius[s+1], y + trunkHeight[s], (float) Math.cos(a1) * trunkRadius[s+1]);
                Vector3f v3 = new Vector3f((float) Math.sin(a0) * trunkRadius[s+1], y + trunkHeight[s], (float) Math.cos(a0) * trunkRadius[s+1]);

                modelHelper.addTriangle(v0, v1, v2, trunk1);
                modelHelper.addTriangle(v2, v3, v0, trunk1);
            }
        }
            ...
            for(int b=0; b<leafSubSections; b++){
                for(int i=0; i<leafNGons[s]; i++){
                    ...
                    if(!(s == leafSections-1 && b == leafSubSections-1)) {
                        ...
                        modelHelper.addTriangle(v0, v1, v2, leaves1);
                        modelHelper.addTriangle(v2, v3, v0, leaves1);
                    }else{
                        Vector3f v4 = new Vector3f(0, y + leafHeight[s][b], 0);

                        modelHelper.addTriangle(v0, v1, v4, leaves1);
                    }
                    ...
                    if(b == 0){
                        ...
                        modelHelper.addTriangle(v4, v1, v0, leaves1, normal);
                    }
